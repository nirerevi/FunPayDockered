...
log "Запуск контейнера funpay_bot в фоновом режиме с помощью Docker Compose"
docker compose up --build -d && log "Контейнер запущен в фоновом режиме"

log "Запуск контейнера funpay_bot в интерактивном режиме"
docker compose run --rm -it funpay_bot && log "Контейнер успешно запущен в интерактивном режиме"
